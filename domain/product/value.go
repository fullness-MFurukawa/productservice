...
func NewProductId(value string) (*ProductId, error) {
	...
	if utf8.RuneCountInString(value) != LENGTH {
		return nil, apperrors.NewDomainError(fmt.Sprintf("商品IDの長さは%d文字でなければなりません。", LENGTH))
	}
	// 引数の正規表現(UUID)チェック
	if !regexp.MustCompile(REGEXP).Match([]byte(value)) {
		return nil, apperrors.NewDomainError("商品IDはUUIDの形式でなければなりません。")
	}
...
func NewProductName(value string) (*ProductName, error) {
	...
	if utf8.RuneCountInString(value) > LENGTH {
		return nil, apperrors.NewDomainError(fmt.Sprintf("商品名の長さは%d文字以内です。", LENGTH))
	}
...
func NewProductPrice(value uint32) (*ProductPrice, error) {
	...
	if value >= MIN && value <= MAX {
		...
		return nil, apperrors.NewDomainError(fmt.Sprintf("単価は%d以上、%d以下です。", MIN, MAX))
	}
}
